...
private:
    ...
    int dp[MAX_L][MAX_L][MAX_L][MAX_L];
    string str1, str2;
    bool judge( int a_b, int a_e, int b_b, int b_e ){
        if( a_b+1 == a_e ){
            return dp[a_b][a_e][b_b][b_e] = (str1[a_b]==str2[b_b]);
        }
        if( dp[a_b][a_e][b_b][b_e] != -1 ){
            return dp[a_b][a_e][b_b][b_e] == 1;
        }
        int res=0;
        int len = a_e - a_b;
        for( int i=1; i<len; ++i ){
        ...
        return dp[a_b][a_e][b_b][b_e] = (res==1) ;
    }
};
int main(){
    Solution sl;
